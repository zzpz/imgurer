...
def difference_hash_n_bits(
    image: Image, hash_size: int = 8, row: bool = True, col: bool = True
) -> (int, int):
    """
    ...
    return (
        row_hash,
        col_hash,
    )  # lets us use either or or concat later or compare separate
...
def parse_image(image_url: str, filename: str) -> ImageCreate:
    ...
    img_create = ImageCreate(
        filename=filename, url=image_url, thumb_url=thumb_url, dhash128=dhash128
    )
    return img_create
